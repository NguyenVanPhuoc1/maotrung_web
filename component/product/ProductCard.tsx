...
import { Eye, Heart, Scan, ChevronRight } from 'lucide-react';
...
export default function ProductCard({
  ...
  return (
    <div className={`group bg-white rounded-lg shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex h-full ${isList ? 'flex-col sm:flex-row' : 'flex-col'}`}> 
      
      {/* 1. IMAGE SECTION (40% in list view) */}
      <div className={`relative overflow-hidden ${isList ? 'w-full sm:w-[40%] aspect-[5/3] sm:aspect-auto' : 'w-full aspect-[5/3]'}`}>
      ...
      <div className={`${isList ? 'w-full sm:w-[60%] flex flex-col' : 'w-full flex-1 flex flex-col'}`}>
        ...
        <div className="p-4 flex gap-4 items-center border-b border-gray-50">
            ...
            <div className="flex-shrink-0">
                ...
                <p className="text-[10px] mt-1 text-gray-500 font-bold uppercase">{code}</p>
            </div>
        ...
        <div className="p-4 flex-1 flex flex-col">
            {/* Product Name */}
            <Link href={`#`} className="text-base font-bold text-[#092F49] hover:text-blue-600 line-clamp-2 mb-3 transition-colors uppercase tracking-tight">
                {name}
            </Link>
            ...
            <div className="mt-auto space-y-3">
                <div className="flex flex-wrap items-baseline gap-2">
                    <span className="text-xl font-extrabold text-[#E31B23]">
                        {price.toLocaleString()}đ<span className="text-sm font-normal text-gray-500 ml-1">/m²</span>
                    </span>
                    {originalPrice && (
